...
public class CustomWallMaker : MonoBehaviour
{
    ...
    public float spawnFromUser = 30f;
    ...
    private const float longPress = 2f;
    private int pressCount = 0;
    private float lastPressTime = 0f;
    private List<Vector3> initialPositions = new List<Vector3>();

    // Number of Points for custom wall, in future can be increased to >10 via UI input
    private const int maximumPoints = 10;

    // Move static counter to class level
    private int pointCounter = 1;

    // Track spawned objects
    private List<GameObject> spawnedPointsCustom = new List<GameObject>();
    private GameObject generatedWall;
    private GameObject buildModeIndication;

    private void Start()
    {
        // Create the build mode indicator
        buildModeIndication = GameObject.CreatePrimitive(PrimitiveType.Sphere);
        buildModeIndication.transform.localScale = new Vector3(0.05f, 0.05f, 0.05f); // Small dot
        buildModeIndication.GetComponent<Renderer>().material = new Material(wallMaterial);
        buildModeIndication.GetComponent<Renderer>().material.color = Color.red; // Set initial color
        buildModeIndication.GetComponent<Collider>().enabled = false; // Disable collider
        buildModeIndication.SetActive(false); // Initially hidden
    }

    private void Update()
    {
        // Long press for entering custom wall build mode
        if (OVRInput.Get(OVRInput.Button.One))
        {
            pressTime += Time.deltaTime;
            // Check if we have entered it or not
            if (pressTime >= longPress)
            {
                // It can be toggled in between the states
                buildMode = !buildMode;
                pressTime = 0f; // Reset timer after toggling
                Debug.Log($"Build Mode: {buildMode}");
                // Update indicator color and visibility
                buildModeIndication.SetActive(true);
                buildModeIndication.GetComponent<Renderer>().material.color = buildMode ? Color.green : Color.red;
                // Generate wall if in build mode and points exist
                if (!buildMode && spawnedPointsCustom.Count > 1)
                {
                    GenerateWall();
        ...
        else
        {
            ...
            if (pressTime > 0f && pressTime < longPress && buildMode)
            {
                // Only spawn if below max points
                if (spawnedPointsCustom.Count < maximumPoints)
                {
                    Debug.Log("Short press detected in build mode. Spawning object.");
                    SpawnObject();
                    // Update initial positions for the new point
                    if (spawnedPointsCustom.Count > initialPositions.Count)
                    {
                        initialPositions.Add(spawnedPointsCustom[spawnedPointsCustom.Count - 1].transform.position);
                    }
                ...
                {
                    Debug.Log($"Maximum number of points reached ({maximumPoints}). Cannot spawn more.");
                }
            }
        ...
        if (!buildMode && spawnedPointsCustom.Count > 1)
        {
            ...
            for (int i = 0; i < spawnedPointsCustom.Count; i++)
            {
                Vector3 currentPosition = spawnedPointsCustom[i].transform.position;
        ...
        if (buildModeIndication.activeSelf)
        {
            Transform cameraTransform = Camera.main.transform;
            buildModeIndication.transform.position = cameraTransform.position + cameraTransform.forward * 0.8f + cameraTransform.right * 0.25f + cameraTransform.up * -0.5f;
            float pulse = Mathf.Abs(Mathf.Sin(Time.time * 2f)) * 0.5f + 0.5f; // Pulsing effect
            buildModeIndication.transform.localScale = new Vector3(0.09f, 0.09f, 0.09f) * pulse;
        }
    }
    ...
    {
        initialPositions.Clear();
        foreach (GameObject point in spawnedPointsCustom)
        {
    ...
    private void SpawnObject()
    {
        ...
        if (pointCounter > maximumPoints)
        {
            Debug.Log($"Maximum number of points reached ({maximumPoints}). Cannot spawn more.");
            return;
        }

        Vector3 spawnPosition = centerEye.position + centerEye.forward * spawnFromUser;
        ...
        GameObject newObject = Instantiate(object, spawnPosition, centerEye.rotation);
        newObject.SetActive(true);
        // Track spawned object
        spawnedPointsCustom.Add(newObject);
        ...
        Debug.Log($"Object spawned at: {spawnPosition} with Point #{pointCounter - 1} | {spawnedPointsCustom.Count}/{maximumPoints}");
    ...
    private void GenerateWall()
    {
        ...
        Vector3[] pointPositions = spawnedPointsCustom.Select(p => p.transform.position).ToArray();
        ...
        for (int i = 0; i < spawnedPointsCustom.Count; i++)
        {
            spawnedPointsCustom[i].transform.position = projectedPoints[i];
        }
